...
public class PrePostProcessor {
   ...
   public static float[] NO_MEAN_RGB = new float[] {0.0f, 0.0f, 0.0f};
     ...
     */
    static ArrayList<DetectionResult> nonMaxSuppression(ArrayList<DetectionResult> boxes, int limit, float threshold) {
    ...
    public static ArrayList<DetectionResult> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                DetectionResult result = new DetectionResult(cls, outputs[i*85+4], rect);
                results.add(result);
            }
        }
